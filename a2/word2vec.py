...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    dim=centerWordVec.shape[0]
    y1=softmax((outsideVectors.dot(centerWordVec.reshape(dim,1))).T).T
    loss=-np.log(y1[outsideWordIdx])
    y=np.zeros_like(y1)
    y[outsideWordIdx]=1
    gradCenterVec=(outsideVectors).T.dot(y1-y)
    gradOutsideVecs=(y1-y).dot(centerWordVec.reshape(1,dim))
...
def negSamplingLossAndGradient(
    ...
    loss=-np.log(sigmoid(uo.dot(centerWordVec)))-np.sum(np.log(sigmoid(-outsideVectors[negSampleWordIndices].dot(centerWordVec))))
    gradCenterVec=(sigmoid(uo.dot(centerWordVec))-1)*uo+np.sum((1-sigmoid(-outsideVectors[negSampleWordIndices].dot(centerWordVec))).reshape(K,1)*outsideVectors[negSampleWordIndices],axis=0)
    gradOutsideVecs=np.zeros_like(outsideVectors)
    for i in negSampleWordIndices:
      gradOutsideVecs[i]+=-(sigmoid(-outsideVectors[i].dot(centerWordVec))-1)*centerWordVec
    gradOutsideVecs[outsideWordIdx]+=(sigmoid(uo.dot(centerWordVec))-1)*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centervec=centerWordVectors[word2Ind[currentCenterWord]]
    for i in outsideWords:
      loss1, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centervec, word2Ind[i], outsideVectors, dataset)
      loss+=loss1
