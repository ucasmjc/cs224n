...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(len(unfinished_parses)>0):
      ...
      trans=model.predict(unfinished_parses[:batch_size])
      for i,j in enumerate(mini_batch):
        j.parse_step(trans[i])
        if len(j.stack)==1 and len(j.buffer)==0:
          unfinished_parses.remove(j)
    dependencies=[ps.dependencies for ps in partial_parses]
