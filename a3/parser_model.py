...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight=nn.Parameter(nn.init.xavier_uniform_(torch.zeros(n_features*self.embed_size,hidden_size)))
    ...
    def embedding_lookup(self, w):
        ...
        x=torch.tensor([list(torch.index_select(self.embeddings,0,w1).view(self.embed_size*self.n_features)) for w1 in w])
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        logits=self.dropout(nn.ReLU()(self.embedding_lookup(w)@self.embed_to_hidden_weight+self.embed_to_hidden_bias)@self.hidden_to_logits_weight+self.hidden_to_logits_bias)
